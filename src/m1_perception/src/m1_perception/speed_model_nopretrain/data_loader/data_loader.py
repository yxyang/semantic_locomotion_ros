...
def generate_augmented_images(images, augmentations):
  """Augments images and corresponding masks."""
  images_tensor = []
  for image in images:
    image_pil = Image.fromarray((image * 256).astype(np.uint8))
    mask = Image.fromarray(np.zeros_like(image).astype(np.uint8))
    if augmentations:
      for augmentation in np.random.choice(augmentations, 2, replace=False):
        image_pil, mask = augmentation(image_pil, mask)
    image_array = np.array(image_pil, dtype=np.uint8).astype(np.float32) / 255.
    images_tensor.append(image_array)

  return np.stack(images_tensor, axis=0)
...
class DataLoader:
  ...
  def next_batch(self):
    ...
    return generate_augmented_images(images, self._augmentations), speed
