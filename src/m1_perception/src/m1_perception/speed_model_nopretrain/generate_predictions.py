...
from m1_perception.speed_model_nopretrain.data_loader.data_utils import recursive_glob
from m1_perception.speed_model_nopretrain.model import HardNet
from m1_perception.speed_model import mask_utils

flags.DEFINE_string('model_dir',
                    'speed_model_logs/nopretrain/trial1/cp-59.ckpt',
...
flags.DEFINE_string(
    ...
    'ghost_memory_taylor_demo2/predictions_nopretrain', 'output paths.')
FLAGS = flags.FLAGS
  ...
  for image_path in tqdm(image_paths):
    ...
    if mask is None:
      mask = mask_utils.get_segmentation_mask(height=image.shape[1],
                                              width=image.shape[2])
    pred_speed_per_pixel = model(image).numpy()[0]
