  ...
  def transform(self, blk, trt=False):
    ...
    for i in range(len(self.conv_layers)):
      ...
      link_ch = [
          ...
          if k > 0 else blk.layers[0][0].weight.shape[1] for k in link
      ]
      ...
      if b_src is not None:
        if trt:
          ...
          #for pytorch, add bias with standalone tensor is more efficient
          # than within conv.bias.
      ...
      if len(link) > 1:
        for j in range(1, len(link)):
          ...
          chie = chis + link_ch[j]
          self.conv_layers[ly].weight[
              chos:choe, :, :, :] = w_src[:, chis:chie, :, :]
      ...
      if isinstance(blk.layers[i][1], nn.BatchNorm2d):
        self.bnrelu_layers[i] = nn.Sequential(blk.layers[i][1],
                                              blk.layers[i][2])
      else:
  ...
  def v2_transform(self, trt=False):
    for i in range(len(self.base)):
      if isinstance(self.base[i], HarDBlock):
        blk = self.base[i]
        self.base[i] = HarDBlock_v2(blk.in_channels, blk.growth_rate,
                                    blk.grmul, blk.n_layers)
        self.base[i].transform(blk, trt)

    for i in range(self.n_blocks):
      blk = self.denseBlocksUp[i]
      self.denseBlocksUp[i] = HarDBlock_v2(blk.in_channels, blk.growth_rate,
                                           blk.grmul, blk.n_layers)
