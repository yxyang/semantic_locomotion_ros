...
from perception.augmentations import get_composed_augmentations
from perception.loader import get_loader
from perception.loss import get_loss_function
from perception.metrics import AverageMeter, RunningScore
from perception.models import get_model
from perception.optimizers import get_optimizer
from perception.schedulers import get_scheduler
from perception.utils import get_logger
...
def train(cfg, writer, logger):
  ...
  if cfg["training"]["resume"] is not None:
    if os.path.isfile(cfg["training"]["resume"]):
      logger.info("Loading model and optimizer from checkpoint '{}'".format(
          cfg["training"]["resume"]))
  ...
  while i <= cfg["training"]["train_iters"] and flag:
    for (images, labels, _) in trainloader:
      ...
      loss = loss_fn(prediction=outputs, target=labels)
      loss.backward()
      optimizer.step()
      ...
      if (i % cfg["training"]["val_interval"]
        ...
        loss_n = 0
        with torch.no_grad():
          for images_val, labels_val, _ in tqdm(valloader):
            images_val = images_val.to(device)
            labels_val = labels_val.to(device)

            outputs = model(images_val)
            val_loss = loss_fn(prediction=outputs, target=labels_val)
...
def main(_):
  logging.info("Logging to: {}".format(FLAGS.logdir))
  writer = SummaryWriter(log_dir=FLAGS.logdir)

  logger = get_logger(FLAGS.logdir)
  logger.info("Let the games begin")

  train(FLAGS.config, writer, logger)
